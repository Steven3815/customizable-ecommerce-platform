...
CREATE TABLE PRODUCT (
    ...
    FOREIGN KEY(store_id) REFERENCES STORE(store_id),
    FOREIGN KEY(store_id, category_id) REFERENCES CATEGORY(store_id, category_id)
);
...
CREATE TABLE PRODUCT_SPEC (
    ...
    FOREIGN KEY(store_id, product_id) REFERENCES PRODUCT(store_id, product_id)
);
...
CREATE TABLE CART_ITEM (
    ...
    FOREIGN KEY(store_id, product_id) REFERENCES PRODUCT(store_id, product_id),
    FOREIGN KEY(store_id, product_id, spec_id) REFERENCES PRODUCT_SPEC(store_id, product_id, spec_id)
);
...
CREATE TABLE ORDER_ITEM (
    ...
    quantity INT NOT NULL,
    price DECIMAL(10,2) NOT NULL,
    FOREIGN KEY(order_id, store_id) REFERENCES ORDERS(order_id, store_id),
    FOREIGN KEY(store_id, product_id, spec_id) REFERENCES PRODUCT_SPEC(store_id, product_id, spec_id)
);
...
CREATE TABLE PAYMENT (
    ...
    payment_confirm_status ENUM('waiting', 'confirmed', 'rejected'),
    payment_note TEXT,
...
CREATE TABLE STORE_SETTING (
    ...
    store_mode ENUM('shopping', 'showcase') DEFAULT 'shopping',
...
CREATE TABLE STORE_PAYMENT_ACCOUNT (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY(store_id, store_payment_id) REFERENCES STORE_PAYMENT_METHOD(store_id, store_payment_id)
);
...
CREATE TABLE PROMOTION_BANNER (
    ...
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY(store_id) REFERENCES STORE(store_id),
    FOREIGN KEY(default_banner_id) REFERENCES DEFAULT_BANNER(default_banner_id)
);
